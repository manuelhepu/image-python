...
try:
    ...
    get_ejecucion = """SELECT MAX(execution) FROM trivy_image WHERE image = %s AND tag = %s"""
    ...
    for i in range(0, totalvuln):
        ...
        postgres_insert_query = """ INSERT INTO trivy_image (image, tag, target, type, vid, package, installed, fixed, severity, url, execution, insertiondate) VALUES (%s, %s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) """
        record_to_insert = (image, tag, vuln["vulnerabilities"][i]["Target"],vuln["vulnerabilities"][i]["Type"],vuln["vulnerabilities"][i]["ID"],vuln["vulnerabilities"][i]["PkgName"],vuln["vulnerabilities"][i]["InstalledVersion"],vuln["vulnerabilities"][i]["FixedVersion"],vuln["vulnerabilities"][i]["Severity"],vuln["vulnerabilities"][i]["PrimaryURL"],execution,dt)
